...
connection_str = f"DRIVER={{ODBC Driver 17 for SQL Server}};SERVER={server};DATABASE={database};UID={user};PWD={password};MultipleActiveResultSets=True;autocommit=False "
...
UPDATE_PRODUCT = "UPDATE scraper_competitor_products_new SET url = ?, alternate_names = ?, competitor = ?, cutsheet_url = ?, description = ?, long_description = ?, manufacturers = ?, their_name = ?, updated_at = CURRENT_TIMESTAMP, extracted_name = ?\
...
UPDATE_IMAGE = "UPDATE scraper_competitor_product_images_new SET src = ?, alt = ?, title = ?, updated_at = CURRENT_TIMESTAMP\
...
UPDATE_OFFER = "UPDATE scraper_competitor_product_offers_new SET condition_id = ?, updated_at = CURRENT_TIMESTAMP, stock = ?, price = ?, currency = ?\
    WHERE scraper_competitor_product_id = ?;"
...
def send_slack_message(message):
    ...
    response = requests.post(
        'https://hooks.slack.com/services/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', data=payload)
    return response
...
def get_products_chunk_from_mongo(data_size=DOCS_CHUNK_SIZE):
    try:
        ...
        print("Docs to skip: ", docs_to_skip, " Results length: ", len(results))
        return results
