...
UPDATE_PRODUCT = "UPDATE scraper_competitor_products_new WITH (ROWLOCK) SET url = ?, alternate_names = ?, competitor = ?, cutsheet_url = ?, description = ?, long_description = ?, manufacturers = ?, their_name = ?, updated_at = CURRENT_TIMESTAMP, extracted_name = ?\
...
UPDATE_IMAGE = "UPDATE scraper_competitor_product_images_new WITH (ROWLOCK) SET src = ?, alt = ?, title = ?, updated_at = CURRENT_TIMESTAMP\
...
UPDATE_OFFER = "UPDATE scraper_competitor_product_offers_new WITH (ROWLOCK) SET condition_id = ?, updated_at = CURRENT_TIMESTAMP, stock = ?, price = ?, currency = ?\
    WHERE scraper_competitor_product_id = ?;"
def get_new_cursor():
